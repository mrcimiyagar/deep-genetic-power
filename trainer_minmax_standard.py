...
def create_model(index, last_layer_units, init_mode, activation, last_activation):
    ...
    kmodel = Dense(dense_units, activation=activation, kernel_initializer=init_mode)(kmodel)
    kmodel = Dense(dense_units, activation=activation, kernel_initializer=init_mode)(kmodel)
...
# joblib.dump(svm, 'SvmModel_minmax_standard')
svm = joblib.load('SvmModel_minmax_standard')

tester = Tester('result_minmax_standard')
tester.test_neuron(model, test_x, test_y, batch_size)
tester.test_svm(model, test_x, test_y, batch_size, svm, [scaler_minmax, scaler_standard])
